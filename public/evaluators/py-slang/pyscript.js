(function () {
        ...
        this.__serviceHandlers = new Map([
            ...
            function helloServiceHandler(message) {
              if (message.data.version < 0 /* Constant.PROTOCOL_MIN_VERSION */) {
                this.__serviceChannel.send(
                  new AbortServiceMessage(0 /* Constant.PROTOCOL_MIN_VERSION */)
                );
                console.error(
                  `Host's protocol version (${message.data.version}) must be at least ${0 /* Constant.PROTOCOL_MIN_VERSION */}`
                );
              } else {
          ...
          [
            ...
            function abortServiceHandler(message) {
              console.error(
                `Host expects at least protocol version ${message.data.minVersion}, but we are on version ${0 /* Constant.PROTOCOL_VERSION */}`
              );
  ...
  function extractImports(code) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const importMatch = line.match(
        /^import\s+([a-zA-Z0-9_][a-zA-Z0-9_.]*)(?:\s+as\s+([a-zA-Z0-9_]+))?/
      );
      if (importMatch) {
        imports.push({
  ...
  class PackageManager {
    ...
    async loadPackagesFromCode(code) {
      ...
      if (this.config.verbose) {
        console.log(
          `[PackageManager] Found ${packageNames.size} packages:`,
      ...
      if (this.config.verbose) {
        ...
        console.log(
          `[PackageManager] Loaded ${successful}/${results.length} packages in ${totalTime}ms`
        );
      }
      return results;
    }
    ...
    async loadWithPyodide(packageName) {
      ...
      if (this.config.verbose) {
        console.log(
          `[PackageManager] Loading ${packageName} (${actualPackageName}) via Pyodide...`
        );
      }
      try {
  ...
  class PythonEvaluator extends BasicEvaluator {
    ...
    async evaluateChunk(chunk) {
      console.log(
        '[PythonEvaluator] Evaluating chunk:',

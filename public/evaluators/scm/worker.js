(function () {
    ...
    ***************************************************************************** */
        ...
        try {
            ...
            steps.push({
                control: expressions.map(expr => {
                    if (expr.constructor.name === 'Application') {
                        const app = expr;
                        return `(${app.operator.name} ${app.operands.map((op) => op.value || op.name || op.toString()).join(' ')})`;
                    }
            ...
            if (expressions.length > 0) {
                ...
                if (expr.constructor.name === 'Application') {
                    ...
                    steps.push({
                        control: [
                            app.operands[0].value || app.operands[0].name || app.operands[0].toString(),
                            app.operands[1].value || app.operands[1].name || app.operands[1].toString(),
                            app.operator.name
                        ],
                        stash: [],
                        environment: {},
                    ...
                    if (app.operands[0].constructor.name === 'Application') {
                        steps.push({
                            control: [
                                app.operands[0].operands[0].value || app.operands[0].operands[0].name,
                                app.operands[0].operands[1].value || app.operands[0].operands[1].name,
                                app.operands[0].operator.name,
                                app.operands[1].value || app.operands[1].name,
                                app.operator.name
                            ],
                            stash: [],
                            environment: {},
     ...
     */
    function* generateCSEMachineStateStream(expressions, maxSteps, control, stash, environment, runtime) {
        const code = expressions.map(expr => expr.toString()).join('\n');
    ...
    // Web Worker entry point for scm-slang conductor runner
    // Note: in worker context, `self` is the link used by Conduit
    initialise(SchemeEvaluator);

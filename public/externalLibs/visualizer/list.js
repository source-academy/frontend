...
export function build_list(n, fun) {
    if (typeof n !== 'number' || n < 0 || Math.floor(n) !== n) {
        throw new Error('build_list(n, fun) expects a positive integer as ' + 'argument n, but encountered ' + n);
    }
    // tslint:disable-next-line:ban-types
...
export function enum_list(start, end) {
    if (typeof start !== 'number') {
        throw new Error('enum_list(start, end) expects a number as argument start, but encountered ' + start);
    }
    if (typeof end !== 'number') {
        throw new Error('enum_list(start, end) expects a number as argument start, but encountered ' + end);
    }
    if (start > end) {
        return null;
    }

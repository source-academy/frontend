...
export default function (StoryXMLPlayer, story, username, attemptedAll) {
    ...
    function initialize(div, canvas) {
        ...
        const now = new Date();
        const mock_act_A1_deployDate = new Date("August 14, 2019 20:30:00");
        const mock_mission_M1_deployDate = new Date("August 14, 2019 21:00:00");
        if (mock_act_A1_deployDate <= now && now < mock_mission_M1_deployDate) {
            StoryXMLPlayer.loadStory('act-A1', function () {
            });
        } else if (mock_mission_M1_deployDate <= now) {

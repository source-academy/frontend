...
import { WorkspaceState } from '../workspace/WorkspaceStateTypes';

export function selectWorkspace<T extends NonStoryWorkspaceLocation>(
  location: T,
...
export function selectWorkspace(
  ...
  selector?: (workspace: WorkspaceState) => any
): SelectEffect;
export function selectWorkspace(
  location: SideContentLocation,
  selector?: (workspace: WorkspaceState) => any
...
export const selectFileSystem = () =>
  select((state: OverallState) => state.fileSystem.inBrowserFileSystem);
export const selectSession = (selector?: (state: SessionState) => any) =>
  select((state: OverallState) => (selector ? selector(state.session) : state.session));

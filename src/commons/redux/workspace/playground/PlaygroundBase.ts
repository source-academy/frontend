...
export const playgroundBaseActions = addLocation<
  typeof playgroundBaseActionsInternal,
  PlaygroundWorkspaces
>(playgroundBaseActionsInternal);
...
>(
  ...
  createWorkspaceSlice(name, initialState, reducers, builder => {
    reducerBuilder(builder);
    if (extraReducers) extraReducers(builder);
  });

...
import { defaultRepl } from '../WorkspaceStateTypes';
...
export const replReducer = createReducer(defaultRepl, builder => {
  ...
  builder.addCase(replActions.clearReplInput, state => {
    state.replValue = '';
  });
  builder.addCase(replActions.clearReplOutput, state => {
    state.output = [];
  });
  builder.addCase(replActions.clearReplOutputLast, state => {
    state.output.pop();
  });
  builder.addCase(replActions.evalInterpreterError, (state, { payload }) => {
    ...
    state.output.push({
      ...
      errors: payload,
      consoleLogs:
        lastOutput !== undefined && lastOutput.type === 'running' ? lastOutput.consoleLogs : []
    });
  });

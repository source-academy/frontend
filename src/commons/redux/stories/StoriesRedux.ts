...
const { actions: reducerActions, reducer: storiesReducer } = createSlice({
  ...
  reducers: {
    addStoryEnv: {
      prepare: (env: string, chapter: Chapter, variant: Variant) => ({
        payload: { env, chapter, variant }
      }),
      reducer(
        state,
        { payload }: PayloadAction<{ env: string; chapter: Chapter; variant: Variant }>
      ) {
        state.envs[payload.env] = getDefaultStoriesEnv(
          payload.env,
          payload.chapter,
          payload.variant
        );
    ...
    setCurrentStoriesGroup: {
      prepare: (id?: number, name?: string, role?: StoriesRole) => ({
        payload: { id, name, role }
      }),
      reducer(
        state,
        { payload }: PayloadAction<{ id?: number; name?: string; role?: StoriesRole }>
      ) {

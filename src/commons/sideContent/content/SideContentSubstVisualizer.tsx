...
import { IStepperPropContents, redex } from 'js-slang/dist/stepper/stepperV2';
...
const SideContentSubstVisualizer: React.FC<SubstVisualizerPropsAST> = props => {
  ...
  return (
    ...
    >
      ...
      <br />
      {hasRunCode ? <StepperDisplayer {...getAST(stepValue)} /> : <SubstDefaultText />}
            ...
            <Popover
              ...
              content={
                <div className=".bp5-running-text {{.modifier}}">
                  <Callout title={node.type} icon="function" intent={getNodeType(node) === "beforeMarker" ? "warning" : "success"}>
                    <div>
                      <span>{"Contraction rule "}</span>
                      <code className="bp5-code">{getNodeType(node) === "beforeMarker" ? "E1 -> E2" : "finished"}</code>
                    </div>
                    <Popover interactionKind='click' placement='left' minimal={true} 
                      content={<div className='.bp5-running-text {{.modifier}}'>
                          <pre className="bp5-code-block">
                          <code>{JSON.stringify(node, null, 4)}</code>
                        </pre>
                      </div>}
                    >
                      <Button icon="eye-open">View AST</Button> 
                    </Popover>
                  </Callout>
                </div>
              }

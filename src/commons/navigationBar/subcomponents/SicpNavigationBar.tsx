...
const SicpNavigationBar: React.FC = () => {
  ...
  /*
    const xhr = new XMLHttpRequest();
    const url = Constants.sicpBackendUrl + "json/searchData.json";
    //console.log(Constants.sicpBackendUrl);
    xhr.open('GET', url, false); //sync download
    xhr.send();
    if(xhr.status !== 200){
      alert('Unable to get search data. Error code = ' +xhr.status);
    ...
    const searchData = JSON.parse(xhr.responseText);
    const indexTrie = searchData['indexSearch'];
    const textbook = searchData['textbook'];
    const textTrie = searchData['userSearch'];
    */
  //const extension = '.json';
  //const [searchData, setSearchData] = React.useState(null);
  //const sicpBackendUrl = 'http://localhost:8080/' || 'https://sicp.sourceacademy.org/';
  
  /* React.useEffect(() => {
  fetch(baseUrl + "trieTreeIndex" + extension)
...
/*
  interface TrieNode {
  [key: string]: TrieNode| undefined | string;
} */
...
/* function autoComplete(query: string) {
  return ["dumb autocomplete result"];
} */
...
const voidSearch = (str :string) => {
    function lineMap(array: any[]) {
      ...
      return {title:textbook[array[0]][array[1]][array[2]], url:SearchUrl + array[0] + array[1]};
    }
    const SearchUrl = ".."
    ...
    //console.log("ans is: " + globalAns);
    //console.log("its length is" + globalAns.length)
    /*
    for(let i=1; i<words.length; i++) {
      globalAns = globalAns.filter((obj : any) => obj.title.includes(words[i]));
...
function sentenceAutoComplete(str : string,limit : number,jsonData : any) {
  const words = str.toLowerCase().split(" ").filter((word) => word !== "");
  if(words.length <2) {
  ...
  lastwords = lastwords.filter((word:string) => preResults.filter((obj: any) => obj.title.toLowerCase().includes(pre + " " + word)).length > 0);
  ...
  const handleAutoIndexSearch = (str:string) => {
    ...
    for(let i=0; i<subindex.length; i++) {
      tem.push({ title: ans['value'] + ': ' + subindex[i]['value'], url: SearchUrl + subindex[i]['id'][0] + subindex[i]['id'][1]});
    }
    }
    setQueryResult(tem);
  }
 ...
 /* let searchMode : string = "user";
  const setSearchMode = (mode : any) => {if(typeof(mode)==='string'){console.log(mode);searchMode = mode;}};
  const getSearchMode = () => searchMode;*/

 /* const userSearch = (
    <div className = "userSearch" style = {{"position" : "absolute", "top" : "10%", "left" : "25%", "width" : "20%", "height" : "600%"}}>
      <div className = "userSearch-inner">
        <div style = {{"display":"inline-flex"}}>
          ...
          />
          <select onChange= {event => setSearchMode(event.target.value)} style = {{"marginLeft" : "25px", "marginRight" : "25px"}}>
            <option value = {"user"}>User Search</option>
            <option value = {"index"}>Index Search</option>
          </select>
          <ControlButton
            label="Search"
            icon={IconNames.SEARCH}
            onClick={() => {getSearchMode() === "user" ? handleSearchButton() : handleIndexSearchButton()}}
          />
        </div>
      </div>
      {searchAutocompleteResults.length !== 0 &&
      <div className="userSearchDropdown" style = {{"backgroundColor" : "white", "outline" : "dashed", "height" : "100%", "overflow" : "auto"}}>
        {searchAutocompleteResults.map((result, index) => (
          <div style = {{"margin" : "2px 2px 3px 3px", "cursor" : "pointer"}} onClick = {()=>setSearchQuery(result)} onMouseOver = {(e)=>{const element = e!.nativeEvent!.srcElement as any; element.style.backgroundColor = 'rgba(0,0,0,0.5)';}} onMouseOut = {(e)=>{const element = e!.nativeEvent!.srcElement as any; element.style.backgroundColor = 'rgba(0,0,0,0)';}}>{result}</div>
        ))}
      </div>}
    </div>
  );*/
  const userSearch = (
    <div className = "userSearch" style = {{"position" : "absolute", "top" : "10%", "left" : "15%", "width" : "20%", "height" : "600%"}}>
      <div className = "userSearch-inner">
        <div style = {{"display":"inline-flex"}}>
          <InputGroup
            placeholder="..."
      ...
      <div className="userSearchDropdown" style = {{"backgroundColor" : "white", "outline" : "dashed", "height" : "100%", "overflow" : "auto"}}>
        {searchAutocompleteResults.map((result, index) => (
          <div style = {{"margin" : "2px 2px 3px 3px", "cursor" : "pointer"}} onClick = {()=>{setSearchQuery(result);  handleAutoSearch(result);}} onMouseOver = {(e)=>{const element = e!.nativeEvent!.srcElement as any; element.style.backgroundColor = 'rgba(0,0,0,0.5)';}} onMouseOut = {(e)=>{const element = e!.nativeEvent!.srcElement as any; element.style.backgroundColor = 'rgba(0,0,0,0)';}}>{result}</div>
        ))}
      </div>}
    </div>
  );
  ...
  const indexSearch = (
    <div className = "indexSearch" style = {{"position" : "absolute", "top" : "10%", "left" : "36%", "width" : "20%", "height" : "600%"}}>
      ...
      <div className="userSearchDropdown" style = {{"backgroundColor" : "white", "outline" : "dashed", "height" : "100%", "overflow" : "auto"}}>
        {indexAutocompleteResults.map((result) => (
          <div style = {{"margin" : "2px 2px 3px 3px", "cursor" : "pointer"}} onClick = {()=> {setIndexSearchQuery(result); handleAutoIndexSearch(result);}} onMouseOver = {(e)=>{console.log(e);const element = e!.nativeEvent!.srcElement as any; element.style.backgroundColor = 'rgba(0,0,0,0.5)';}} onMouseOut = {(e)=>{console.log(e);const element = e!.nativeEvent!.srcElement as any; element.style.backgroundColor = 'rgba(0,0,0,0)';}}>{result}</div>
        ))}
      </div>}
    </div>
  );
  ...
  return (
    <>
      <Navbar className="SicpNavigationBar secondary-navbar">
        ...
        {[tocButton, userSearch, indexSearch]}
        </NavbarGroup>
